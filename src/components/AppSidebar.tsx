...
import { Sparkles, PenTool, Clock, Bookmark, Compass, Crown, Zap } from "lucide-react";
...
const NAV_ITEMS = [
  ...
  { id: "history", icon: Clock, labelEn: "History", labelTr: "Geçmiş", placeholder: true },
  { id: "saved", icon: Bookmark, labelEn: "Saved", labelTr: "Kaydedilenler", placeholder: true },
  { id: "discover", icon: Compass, labelEn: "Discover", labelTr: "Keşfet", placeholder: true },
];
        ...
        {NAV_ITEMS.map((item) => {
          ...
          return (
            <button
              key={item.id}
              onClick={() => {
                if (!item.placeholder && onNavChange) onNavChange(item.id);
                if (item.placeholder) {
                  // placeholder items - show toast or do nothing
                }
              }}
